...
def main():
	...
	screen = pygame.display.set_mode((windows_width, windows_height)) #创建一块屏幕

	pygame.display.set_caption("Python贪吃蛇")	#设置窗口标题
...
def running_game(screen,snake_speed_clock):
	...
	while True:
		...
		snake_speed_clock.tick(snake_speed) #控制fps,每秒内循环要运行的次数
#使用def函数将食物画出来
def draw_food(screen, food):
	x = food['x'] * cell_size
	y = food['y'] * cell_size
	appleRect = pygame.Rect(x, y, cell_size, cell_size)	#属性pygame.Rect(left, top, width, height)
...
def draw_snake(screen, snake_coords):
	for coord in snake_coords:
		...
		wormInnerSegmentRect = pygame.Rect(                #蛇身子里面的第二层蓝色
...
def show_start_info(screen):
	...
	gamestart = pygame.image.load('welcome.jpg')
	screen.blit(gamestart, (0, 0))
	screen.blit(tip, (240, 450))
